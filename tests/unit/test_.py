...
def test_get_score(mock_storage_manager, login, first_name, last_name, email, phone, birthday, gender, expected_result):
    ...
    result = get_score(mock_storage_manager, request_data, login=login)

    assert result == expected_result
    assert result.get("score") == mock_storage_manager.client.get(key)
...
def test_get_admin_score(
        mock_storage_manager, login, first_name, last_name, email, phone, birthday, gender, expected_result
):
    request_data = OnlineScoreRequest(
        first_name=first_name,
        last_name=last_name,
...
def test_get_interests(mock_storage_manager, login, client_ids, date):
    ...
    for client_id in data_to_insert:
        mock_storage_manager.set_list_data(client_id, data_to_insert[client_id])

    result = get_interests(mock_storage_manager, request_data, login)

    assert result == data_to_insert


def test_get_admin_interests(mock_storage_manager):
    request_data = ClientsInterestsRequest(client_ids=[0], date="01.01.2000")

    result = get_interests(mock_storage_manager, request_data, "admin")
